...
let arrProdName = ["банан", "помидор", "хлеб", "карандаш", "рыба", "лазер", "чеснок", "дефебрилятор", "хз что это", "куртка", "хрен"];
...
for (var i = 0; i < 11; i++) {
    let prodPrice = Math.round(Math.random() * 400);
...
for (var i = 0; i < arrProductsInShop.length; i++) {
    ...
    pDesc.textContent = "<Какое-нибуть незамысловатое описание товара>";
    ...
    .addEventListener("click", (e) => {
        ...
        if (name) {
            ...
            arrProductsInShop.forEach((el) => {
                if (el.name == name) {
                    userBasket.push(el);
                    //alert("Tовар добавлен в корзину");
                }
            })
            userBasket.fillAddress(); // добавляет кнопку Next
            userBasket._visibility = true;
